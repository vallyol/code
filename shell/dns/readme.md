...
Основа - это использование `dig`, так же как это делается в скрипте dns-test.sh. С небольшим отличием: для используемого домена выводятся две колонки `RealTime` и `RCODE`. Колонка `FakeRCODE` связана с фейковым доменом. Фейковый домен нужен для того, чтобы определить, как ДНС сервер реагирует на неусуществующий домен

### Пример вывода в терминале

```
$ ./dns-spoofing.sh youtube.com

=== ЗАПУСК ТЕСТА DNS ===
Целевой домен: youtube.com
Тестовый домен: check-hijack-1784439731845373373.test

Сервис             IP Сервера      Статус   RealTime   RCODE      FakeRCODE  Проверка
---------------------------------------------------------------------------------------
Current DNS        192.168.43.1    OK       4 msec     NOERROR    NOERROR    Нет    
---------------------------------------------------------------------------------------
AdGuard DNS        94.140.14.14    OK       96 msec    REFUSED    NXDOMAIN   Нет    
Comodo Secure      8.26.56.26      OK       96 msec    REFUSED    N/A        Неопределено
Yandex DNS         77.88.8.8       OK       44 msec    REFUSED    NXDOMAIN   Нет    
Control D          76.76.2.4       OK       116 msec   REFUSED    NXDOMAIN   Нет    
Alibaba            223.5.5.5       OK       36 msec    NOERROR    SERVFAIL   Неопределено
Neustar Ultra      156.154.70.5    OK       96 msec    REFUSED    NXDOMAIN   Нет    
CleanBrowsing      185.228.168.9   OK       84 msec    NOERROR    NXDOMAIN   Нет    
OpenDNS            208.67.222.222  OK       96 msec    NOERROR    NXDOMAIN   Нет    
Google DNS         8.8.8.8         OK       52 msec    NOERROR    NXDOMAIN   Нет    
Quad9              9.9.9.9         OK       80 msec    REFUSED    REFUSED    Неопределено
Mullvad            194.242.2.2     OK       96 msec    REFUSED    REFUSED    Неопределено
Cloudflare DNS     1.1.1.1         OK       76 msec    NOERROR    SERVFAIL   Неопределено
```

Здесь в качестве целевого домена используется youtube.com, фейковый домен генерируется прямо в скрипте.

По выводу:
- "IP Сервера" показывает адрес тестируемого ДНС сервера
- статус "ОК" говорит о том, что ДНС сервер ответил
- "RealTime" это время ответа на реальный домен (youtube.com, указывается при запуске скрипта как аргумент)
- "RCODE" ответ на запрос к реальному домену
- "FakeRCODE" ответ на запрос к фейковому домену
- "Проверка" говорит об обнаружении подмены для фейкового домена

### Как понимать результаты...

- `Google DNS`, `OpenDNS`, `CleanBrowsing` и несколько других дают `FakeRCODE = NXDOMAIN`, то есть на фейковый домен они отвечают ожидаемо.
- `Cloudflare DNS` и `Alibaba` показывают `SERVFAIL` на фейк — это уже не стандартный «чистый NXDOMAIN», но и не прямое доказательство подмены.
- `Quad9` и `Mullvad` возвращают `REFUSED` даже на фейк — это скорее политика/ограничение ответа, чем подмена адреса.
- `AdGuard`, `Yandex`, `Control D`, `Neustar` показывают `REFUSED` на реальный домен, а это уже полезный сигнал: они не резолвят запрос в том виде, как он отправлялся, или применяют свои правила.

То есть, логично считать:

- `NXDOMAIN` на fake — нормой;
- `NOERROR` на реальный домен — нормой;
- `A/AAAA` на fake — явной подменой;
